  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 md:px-6 lg:px-8">
          ...
          <Reveal direction="up" delay={200}>
            <p className="mt-5 max-w-2xl text-base text-white/70 leading-relaxed">
              Apple, Mango, Melon, Banana, and Oranges handled with grading, packing, and
              export-ready preparation where required.
            </p>
          </Reveal>
          ...
          <Reveal direction="up" delay={300}>
            <div className="mt-10 flex flex-wrap gap-5">
              {[
                { value: 5,  suffix: '',   label: 'Fruit Varieties' },
      ...
      <main className="bg-slate-50">
        ...
        {/* ── HIGHLIGHTS STRIP ── */}
        <section aria-label="Fruit highlights" className="border-b border-emerald-50 bg-white">
          <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 lg:px-8">
            <div className="flex flex-wrap justify-center gap-8 md:justify-between">
              {[
                { icon: '🍎', label: 'Export-Grade',    desc: 'Packed to international standards' },
                { icon: '📦', label: 'Flexible Lots',   desc: 'Wholesale and spot quantities' },
                { icon: '🌿', label: 'Fresh Grading',   desc: 'Strict quality checks at source' },
                { icon: '🚚', label: 'Fast Dispatch',   desc: 'Time-bound cold-chain logistics' },
              ].map((item, i) => (
                <Reveal key={item.label} delay={i * 90} direction="up">
                  <div className="flex items-center gap-3 min-w-[180px]">
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-xl shrink-0">
                      {item.icon}
                    </span>
                    <div>
        ...
        >
          <Reveal direction="up">
            ...
            <p className="mt-3 max-w-xl text-sm text-slate-600">
              Each fruit is sourced, graded, and packed to meet wholesale and export buyer requirements.
              Request a quote for any item below.
            </p>
          </Reveal>

          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {products.fruits.map((product, i) => (
              ...
              <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                <div>
                  <h2 className="text-xl font-bold tracking-tight md:text-2xl">
                    Looking for a specific fruit variety?
                  </h2>
                  <p className="mt-2 text-sm text-emerald-100 max-w-md">
                    ...
                    on all fruits — with flexible lot sizes and packing options.
                  </p>
                </div>
